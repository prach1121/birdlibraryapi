...
A **Paper** plugin that acts as a JavaScript scripting engine similar to Skript,
but using **real JavaScript** (via [GraalJS](https://www.graalvm.org/javascript/)).
Drop `.js` files into `plugins/BirdApi/` and write events/commands with direct
access to the Bukkit API via Java interop (`Java.type(...)`).
...
│   ├── ScriptManager.java      # scans/loads/reloads .js files
...
└── BirdApi-examples/
    ├── example.js           # working example script
...
This produces `target/BirdLibraryApi.jar` (fairly large, ~30-40MB, since GraalJS
is shaded into it so it's a single self-contained jar with no extra libs needed
on the server).
...
3. Place your `.js` files (e.g. `BirdApi-examples/example.js`) in `plugins/BirdApi/`
...
```
/birdlib reload            -> reload every script
/birdlib reload example    -> reload only example.js (with or without ".js")
/birdlib reload example.js -> same thing
```
...
- Errors that happen while an event handler **runs** (not while loading), such as
  ...
  the server.
- Test this with `BirdApi-examples/broken-example.js`, which has an intentional
  syntax error (a missing `,` before the callback function).
...
willing to write actual JavaScript in exchange for removing those limits:
...
BirdLibraryApi assumes you're comfortable writing JavaScript. If you already
can, BirdLibraryApi removes the ceiling Skript eventually hits.
...
| `Java.type("...")` | GraalJS built-in used to import any Java class into the script |
...
- Each `.js` file gets its own GraalJS context with its own global scope (a `var x`
  in one file never collides with another file's `var x`).
